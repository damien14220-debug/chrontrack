...
function Sport() {
  ...
  const refreshStravaToken = async () => {
    ...
    } catch(e) { console.error('Erreur refresh token Strava:', e) }
    return null
  }
  ...
  const handleStravaCallback = async () => {
    ...
    } catch(e) { console.error('Erreur Strava callback:', e) }
  ...
  const syncStrava = async () => {
    ...
    if (token) {
      ...
      deconnecterStrava()
      alert('Session Strava expirée. Merci de te reconnecter.')
    }
  }
  ...
  return (
    <div className="px-3 py-4 md:px-6 md:py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white mb-1">🏃 Journal Sportif</h2>
          <p className="text-slate-500 dark:text-gray-400 text-sm">Suis tes séances et tes sensations physiques.</p>
        </div>
        <button
      ...
      {/* Bloc Strava */}
      <div className={`rounded-2xl p-4 mb-6 border ${stravaConnecte ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800/40' : 'bg-white dark:bg-gray-900 border-slate-200 dark:border-gray-800'} shadow-sm dark:shadow-none`}>
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-orange-500 rounded-xl flex items-center justify-center text-white font-bold text-lg shrink-0">S</div>
            <div>
              <p className="font-bold text-slate-900 dark:text-white text-sm">Strava</p>
              <p className="text-slate-500 dark:text-gray-400 text-xs">
                {stravaConnecte ? '✅ Connecté' : 'Importe tes activités automatiquement'}
              </p>
            </div>
          </div>
            ...
            {stravaConnecte ? (
              <>
                <button
                  ...
                  className="bg-orange-500 hover:bg-orange-600 text-white font-semibold px-3 py-2 rounded-xl transition text-xs disabled:opacity-50"
                >
                  {stravaLoading ? '⏳' : '🔄 Sync'}
                </button>
                <button
                  onClick={deconnecterStrava}
                  className="bg-slate-100 dark:bg-gray-800 text-slate-500 dark:text-gray-400 font-semibold px-3 py-2 rounded-xl transition text-xs"
                >
                  Déco
                </button>
              </>
      ...
      {/* Formulaire ajout */}
      {showForm && (
        <div className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-2xl p-5 mb-6 shadow-sm dark:shadow-none">
          <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-5">🏋️ Nouvelle séance</h3>
          ...
          <div className="mb-4">
            <label className="text-slate-500 dark:text-gray-400 text-sm mb-2 block">Type de sport</label>
            <div className="flex flex-wrap gap-2">
              {SPORTS.map(s => (
                <button
                  key={s.label}
                  onClick={() => setForm({ ...form, sport: s.label })}
                  className={`flex items-center gap-2 px-3 py-2 rounded-xl border transition font-medium text-sm ${
                    form.sport === s.label
                      ? 'bg-sky-50 dark:bg-sky-500/20 border-sky-300 dark:border-sky-500/50 text-sky-600 dark:text-sky-400'
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div>
              <label className="text-slate-500 dark:text-gray-400 text-sm mb-2 block">Durée (min)</label>
              <input
                type="number"
                value={form.duree}
          ...
          {[
            ...
            <div key={field.key} className="mb-4">
              <label className="text-slate-500 dark:text-gray-400 text-sm mb-2 block font-medium">{field.label}</label>
              <div className="flex gap-1">
                {field.items.map(n => (
                  <button
                    key={n.valeur}
                    onClick={() => setForm({ ...form, [field.key]: n.valeur })}
          ...
          {form.sensation_ventre <= 3 && (
            <div className="mb-4">
              <label className="text-slate-500 dark:text-gray-400 text-sm mb-2 block">Détails symptômes</label>
              <input
                type="text"
                value={form.symptomes_ventre}
                onChange={e => setForm({ ...form, symptomes_ventre: e.target.value })}
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => setShowForm(false)} className="bg-slate-100 dark:bg-gray-800 text-slate-700 dark:text-gray-300 font-semibold px-6 py-3 rounded-xl transition">Annuler</button>
          </div>
        </div>
      )}
            ...
            <>
              ...
              <div className="flex flex-col gap-4">
                {stats.statsParSport.map(s => (
                  <div key={s.sport} className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-2xl p-4 shadow-sm dark:shadow-none">
                    <div className="flex items-center gap-3 mb-4">
                      <span className="text-3xl">{s.emoji}</span>
                      <div className="flex-1">
                        <h3 className="font-bold text-slate-900 dark:text-white">{s.sport}</h3>
                        <p className="text-slate-400 dark:text-gray-500 text-xs">{s.nb} séance{s.nb > 1 ? 's' : ''} {labelPeriode[periodeStats]}</p>
                      </div>
                      {s.mauvaises_seances > 0 && (
                        <span className="bg-red-50 dark:bg-red-500/20 text-red-500 text-xs px-2 py-1 rounded-full border border-red-200 dark:border-red-500/30">
                          ⚠️ {s.mauvaises_seances}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      {s.duree_total > 0 && (
                        <div className="bg-slate-50 dark:bg-gray-800 rounded-xl p-3">
                          ...
                          <p className="font-bold text-slate-900 dark:text-white text-sm">{Math.floor(s.duree_total / 60)}h{s.duree_total % 60}min</p>
                        </div>
                      )}
                      {s.duree_moy && (
                        <div className="bg-slate-50 dark:bg-gray-800 rounded-xl p-3">
                          <p className="text-xs text-slate-400 dark:text-gray-500 mb-1">⏱️ Durée moy.</p>
                          <p className="font-bold text-slate-900 dark:text-white text-sm">{s.duree_moy} min</p>
                        </div>
                      )}
                      {s.distance_total && (
                        <div className="bg-slate-50 dark:bg-gray-800 rounded-xl p-3">
                          <p className="text-xs text-slate-400 dark:text-gray-500 mb-1">📍 Distance totale</p>
                          <p className="font-bold text-slate-900 dark:text-white text-sm">{s.distance_total} km</p>
                        </div>
                      )}
                      {s.distance_moy && (
                        <div className="bg-slate-50 dark:bg-gray-800 rounded-xl p-3">
                          <p className="text-xs text-slate-400 dark:text-gray-500 mb-1">📍 Distance moy.</p>
                          <p className="font-bold text-slate-900 dark:text-white text-sm">{s.distance_moy} km</p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div className="bg-slate-50 dark:bg-gray-800 rounded-xl p-3 text-center">
                        ...
                        <p className={`font-bold text-sm ${getNiveau(Math.round(s.energie_avant_moy)).couleur}`}>{s.energie_avant_moy}/5</p>
                      </div>
                      <div className="bg-slate-50 dark:bg-gray-800 rounded-xl p-3 text-center">
                        <p className="text-xs text-slate-400 dark:text-gray-500 mb-1">Énergie après</p>
                        <p className={`font-bold text-sm ${getNiveau(Math.round(s.energie_apres_moy)).couleur}`}>{s.energie_apres_moy}/5</p>
                      </div>
                      <div className="bg-slate-50 dark:bg-gray-800 rounded-xl p-3 text-center">
                        <p className="text-xs text-slate-400 dark:text-gray-500 mb-1">Ventre moy.</p>
                        <p className={`font-bold text-sm ${getSensation(Math.round(s.sensation_moy)).couleur}`}>{s.sensation_moy}/5</p>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            {groupes.map(([date, seancesJour]) => (
              <div key={date} className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm dark:shadow-none">
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 py-3 hover:bg-slate-50 dark:hover:bg-gray-800 transition"
                >
                  <div className="flex items-center gap-3">
                    <span className="text-xl">📅</span>
                    <div className="text-left">
                      <p className="font-bold text-slate-900 dark:text-white text-sm capitalize">{formatDate(date)}</p>
                      <p className="text-slate-400 dark:text-gray-500 text-xs">{seancesJour.length} séance{seancesJour.length > 1 ? 's' : ''}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {seancesJour.slice(0, 2).map(s => {
                      const badge = getBadge(s)
                      return <span key={s.id} className={`text-xs px-2 py-0.5 rounded-full border hidden sm:inline ${badge.classe}`}>{badge.label}</span>
                    })}
                    <span className="text-slate-400 dark:text-gray-500 text-xs">{joursOuverts[date] ? '▲' : '▼'}</span>
                  </div>
                </button>
                ...
                {joursOuverts[date] && (
                  <div className="border-t border-slate-100 dark:border-gray-800">
                    {seancesJour.map(s => {
                      ...
                      return (
                        <div key={s.id} className="px-4 py-4 border-b border-slate-100 dark:border-gray-800/50 last:border-0">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-3">
                                <span className="text-2xl">{getSportEmoji(s.sport)}</span>
                                <div>
                                  <p className="font-bold text-slate-900 dark:text-white text-sm">{s.sport}</p>
                                  <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-gray-400">
                                    {s.duree && <span>⏱️ {s.duree} min</span>}
                                    {s.distance && <span>📍 {s.distance} km</span>}
                                    {s.note?.includes('Strava') && <span className="text-orange-500 font-medium">🟠 Strava</span>}
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="grid grid-cols-3 gap-2 mb-3">
                                {[
                                  ...
                                  <div key={item.label} className="bg-slate-50 dark:bg-gray-800 rounded-xl p-2 text-center">
                              ...
                              {s.symptomes_ventre && (
                                <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/40 rounded-xl px-3 py-2 mb-2">
                                  <p className="text-red-600 dark:text-red-300 text-xs">🩺 {s.symptomes_ventre}</p>
                                </div>
                              )}
                              {s.note && !s.note.includes('Strava') && (
                                <p className="text-slate-400 dark:text-gray-500 text-xs">📝 {s.note}</p>
                              )}
                            </div>
                            ...
                            <div className="flex flex-col gap-2 ml-3">
                              <button
                                ...
                                className="text-slate-300 dark:text-gray-600 hover:text-sky-400 transition text-lg"
                              >✏️</button>
                              <button
                                onClick={() => handleDelete(s.id)}
                                className="text-slate-300 dark:text-gray-600 hover:text-red-400 transition text-lg"
                              >🗑️</button>
                            </div>
                          </div>
                        </div>
      ...
      {editRessentis && (
        <div className="fixed inset-0 bg-black/50 flex items-end md:items-center justify-center z-50 p-0 md:px-4">
          <div className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-t-2xl md:rounded-2xl p-5 w-full md:max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">

            <div className="flex justify-center mb-3 md:hidden">
              <div className="w-10 h-1 bg-slate-200 dark:bg-gray-700 rounded-full"></div>
            </div>

            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1">✏️ Modifier les ressentis</h3>
            <p className="text-slate-500 dark:text-gray-400 text-xs mb-5">Les données Strava (km, durée) restent inchangées.</p>
              ...
              <div key={field.key} className="mb-4">
                ...
                <div className="flex gap-1">
                  {field.items.map(n => (
                    <button
                      key={n.valeur}
                      onClick={() => setEditForm({ ...editForm, [field.key]: n.valeur })}
                      className={`flex-1 py-2 rounded-xl text-xs font-semibold border transition ${
            ...
            {editForm.sensation_ventre <= 3 && (
              <div className="mb-4">
                ...
                <input
                  type="text"
                  value={editForm.symptomes_ventre}
            ...
            <div className="flex gap-3">
              <button
                onClick={handleEditRessentis}
                className="flex-1 bg-sky-500 hover:bg-sky-600 text-white font-semibold py-3 rounded-xl transition"
              >
                Enregistrer
              </button>
              <button
                onClick={() => setEditRessentis(null)}
                className="flex-1 bg-slate-100 dark:bg-gray-800 text-slate-700 dark:text-gray-300 font-semibold py-3 rounded-xl transition"
              >
                Annuler
              </button>
            </div>
