...
function Assistant() {
  ...
  const fetchContexte = async () => {
    ...
    const il_y_a_30_jours = new Date(aujourd_hui - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
    ...
    const [
      ...
      supabase.from('analyses').select('*').order('date', { ascending: false }).limit(20),
      supabase.from('symptomes').select('*').gte('date', il_y_a_30_jours).order('date', { ascending: false }),
      supabase.from('repas').select('*').gte('date', il_y_a_30_jours).order('date', { ascending: false }).limit(20),
...
${(analyses || []).slice(0, 10).map(a => `- ${a.type}: ${a.valeur} ${a.unite || ''} (le ${new Date(a.date).toLocaleDateString('fr-FR')})`).join('\n')}
...
SYMPTÔMES DES 30 DERNIERS JOURS :
${(symptomes || []).length > 0
  ? symptomes.slice(0, 10).map(s => `- ${s.type} (intensité ${s.intensite}/5) le ${new Date(s.date).toLocaleDateString('fr-FR')}`).join('\n')
  : '- Aucun symptôme enregistré'}

REPAS RÉCENTS :
${(repas || []).length > 0
  ? repas.slice(0, 10).map(r => `- ${r.moment}: ${r.aliments} — réaction: ${r.reaction || 'aucune'} (le ${new Date(r.date).toLocaleDateString('fr-FR')})`).join('\n')
...
`
  ...
  const envoyerMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: reponseIA }])
    } catch (err) {
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] md:h-[calc(100vh-2rem)] px-3 py-4 md:px-6 md:py-8">
      ...
      <div className="mb-4">
        ...
        <p className="text-slate-500 dark:text-gray-400 text-sm">
          Ton assistant personnalisé — il connaît tes analyses, symptômes et repas récents.
        </p>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-emerald-50 dark:bg-green-500/10 border border-emerald-200 dark:border-green-500/30 rounded-2xl p-4">
              ...
              <p className="text-slate-700 dark:text-gray-300 text-sm">
                Bonjour ! Je suis ton assistant spécialisé Crohn. J'ai accès à tes analyses récentes, 
                tes symptômes et tes repas des 30 derniers jours pour te donner des conseils vraiment personnalisés. 
